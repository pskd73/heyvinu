...
int WindowPopulateState(const struct WindowConfig *config, struct WindowState *state, int sample_rate)
{
...
#ifdef USE_ESP32
  state->coefficients = heap_caps_malloc(state->size * sizeof(*state->coefficients), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  ...
  state->input_used = 0;
#ifdef USE_ESP32
  state->input = heap_caps_malloc(state->size * sizeof(*state->input), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (state->input == NULL)
#endif
  {
...
#ifdef USE_ESP32
  state->output = heap_caps_malloc(state->size * sizeof(*state->output), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
