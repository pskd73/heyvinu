...
#ifdef USE_ESP32
  state->input =
      heap_caps_malloc(state->fft_size * sizeof(*state->input), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (state->input == NULL)
#endif
  {
...
#ifdef USE_ESP32
  state->scratch = heap_caps_malloc(scratch_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
